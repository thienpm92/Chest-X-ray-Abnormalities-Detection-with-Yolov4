...
class Yolo_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.cfg.dataset_dir+'/train', img_path)
    ...
    def _get_val_item(self, index):
        ...
        img = cv2.imread(os.path.join(self.cfg.dataset_dir+'/val', img_path))
...
def get_image_id(filename:str) -> int:
    ...
    t = list(filename)
    x = [str(ord(char)) for char in t]
    s = ""
    ids = int(s.join(x[j] for j in range(3)))
    return ids
